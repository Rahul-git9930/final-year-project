...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const userInfo = document.getElementById('userInfo') || document.querySelector('.account');
  if (userInfo && user.email) {
    userInfo.innerHTML = `
      <p>${user.email}</p>
      <small>Role: ${user.role || 'Member'}</small>
    `;
  }

  // Create and setup sidebar menu based on user role
  const isStudent = user.role === 'student' || user.role === 'member';
  const sidebarMenu = document.getElementById('sidebarMenu') || document.querySelector('.sidebar ul');
  ...
  if (sidebarMenu) {
    if (isStudent) {
      ...
      // Admin/Librarian menu - keep existing
      sidebarMenu.innerHTML = `
        <li class="active">Dashboard</li>
        <li>Books</li>
        <li>Users</li>
        ...
        ${books.slice(0, 12).map(book => `
          <div style="background: white; padding: 15px; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); transition: 0.3s;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-top: 10px;">
              ${book.available > 0 ? 
                `<button onclick="requestBook('${book._id}')" style="padding: 8px; background: #198754; color: white; border: none; border-radius: 5px; cursor: pointer; font-weight: bold;">Request Book</button>` :
...
async function loadWishlist() {
  ...
  try {
    ...
    const wishlist = response.ok ? await response.json() : [];
    ...
    content.innerHTML = `
      ...
      <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(250px, 1fr)); gap: 20px; margin-top: 20px;">
        ${wishlist.length > 0 ? wishlist.map(item => `
          <div style="background: white; padding: 15px; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
            <h3 style="margin: 0 0 5px 0; color: #0f5132;">${item.book.title}</h3>
            <p style="margin: 5px 0; color: #666; font-size: 14px;"><strong>by</strong> ${item.book.author}</p>
            <p style="margin: 5px 0; color: #666; font-size: 13px;">Category: ${item.book.category}</p>
            <p style="margin: 10px 0; padding: 8px; background: ${item.book.available > 0 ? '#d4edda' : '#f8d7da'}; border-radius: 5px; text-align: center; font-weight: bold; color: ${item.book.available > 0 ? '#155724' : '#721c24'};">
              ${item.book.available > 0 ? `Available Now! (${item.book.available})` : 'Not Available'}
            </p>
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-top: 10px;">
              ${item.book.available > 0 ? `<button onclick="requestBook('${item.book._id}')" style="padding: 8px; background: #198754; color: white; border: none; border-radius: 5px; cursor: pointer; font-weight: bold;">Request</button>` : ''}
...
async function renewBook(transactionId) {
  ...
  if (!confirm('Are you sure you want to request book return?')) {
    return;
  }
  ...
  if (!confirm('Mark this fine as paid? Please ensure you have paid at the library counter.')) {
    return;
  }
  ...
  try {
    ...
    if (response.ok) {
      ...
      const error = await response.json();
      alert(error.message || 'Failed to mark fine as paid. Please contact the librarian.');
    }
  } catch (error) {
    console.error('Error paying fine:', error);
    alert('Error processing payment. Please contact the librarian.');
  }
}
